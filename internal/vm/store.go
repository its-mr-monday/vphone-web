...
func scanVM(s interface{ Scan(...any) error }) (VM, error) {
	...
	if err := s.Scan(
		...
		&v.CPU, &v.Memory, &v.DiskSize, &v.PortBlockBase, &v.VMDir, &v.PID,
		&v.ErrorMessage, &createdAt, &updatedAt,
	); err != nil {
		return VM{}, err
	}
	var err error
...
const vmColumns = `id, name, status, variant, ios_version, ipsw_id, cpu, memory,
	disk_size, port_block_base, vm_dir, pid, error_message, created_at, updated_at`
...
func (s *store) insert(v VM) error {
	_, err := s.db.Exec(
		`INSERT INTO vms (`+vmColumns+`)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		v.ID, v.Name, v.Status, v.Variant, v.IOSVersion, v.IPSWID, v.CPU, v.Memory,
		v.DiskSize, v.PortBlockBase, v.VMDir, v.PID, v.ErrorMessage,
